...
For each frequency (roughly 30 Hz to 720 Hz), and for each component (TZXR, TZXI, TZYR, TZYI), the data values span roughly the same range of values. Because of this:
...
To keep our synthetic example simple, we choose to add uncertainties of 0.02 to all data. This was roughly 10% the maximum anomaly amplitude for most components and frequencies. Despite being a quick way to assign uncertainties, we may end up over-fitting certain frequencies and/or components at the expense of others. To apply uncertainties:
